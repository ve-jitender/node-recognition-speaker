...
function verifyParameters(expected = [], actual = {}) {
	...
	return new Promise((resolve, reject) => {
		...
		const missingParameters = expected.filter(param => param.required).filter(param => !actual[param.name]);
...
function verifyHeaders(expectedHeaders = [], actualHeaders = {}) {
	return new Promise((resolve, reject) => {
		...
		const requiredPropsNotPresent = expectedHeaders.filter(param => param.required).filter(param => !actualHeaders[param.name]);
